...
export default function CarsList({
  ...
  const { data } = useSWR('/api/cars?' + stringify(query), {
    ...
    // dedupingInterval, which defaults to 2000ms. dedupingInterval means during the interval, all the useSWR with the same key will not fire request.
    initialData: deepEqual(query, serverQuery)
    // deepEqual. Performs a deep equality comparison on the two objects passed and returns a boolean representing the value equivalency of the objects.
      ? { cars, totalPages }
      : undefined,
  });
